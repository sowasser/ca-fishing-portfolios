...
## Overall timeseries
Aggregated across all ports, the timeseries for the number of fishers, total revenue (adjusted for inflation to match the 2014 value), and pounds landed from 1992-2014 shows a general declining trend in fishers and pounds landed. Revenue is highly variable over the timeseries, with a decline around 1997 and short-lived recoveries in 2003-2004 and 2012-2013.
...
## Timeseries by port
There is high variability in the amount of revenue (adjusted for inflation to match the 2014 value) for each port in the dataset. Few ports show any overall trend, with the exception of an upwards trend for Ventura, and a slight decline for the Crecent City Area Ports, the Los Angeles - Long Beach Area Ports, and the San Francisco Area Ports.
...
pl <- pl[order(pl$year, pl$port, -pl$revenue_adj), ]  # order by revenue

# Total revenue per port per year
pl_revenue <- group_by(pl, year, port) %>% summarize(revenue = sum(revenue_adj))
...
fish_sums <- group_by(port_landings, year, port, fishery) %>% 
  summarize(revenue = sum(revenue_adj))
...
Overall, for each year, the most valuable fishery (adjusted for inflation to match the 2014 value) was most often market squid (12 years), followed by dungeness crab (8 years), herring roe (2 years), and shrimp/prawn (1 year).
...
pl_max_yr <- pl %>% 
  arrange(year, desc(revenue_adj)) %>% 
  group_by(year) %>%
  slice(1:1)

max_revenue_yr <- ggplot(pl_max_yr, aes(y = revenue_adj, x = year, fill = fishery)) +
